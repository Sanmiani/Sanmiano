...
description: Use when running HyperFrames CLI commands — init, lint, inspect, preview, render, tts, transcribe, remove-background, doctor, or any npx hyperframes command
---
...
When using `--tailwind`, invoke the `tailwind` skill before editing classes or theme tokens.
...
Reports: text outside containers, text clipped by fixed boxes, text outside canvas, children escaping clip containers.

Mark intentional overflow: `data-layout-allow-overflow`. Mark decorative elements to skip: `data-layout-ignore`.
...
Hot-reloads on file changes. When handing a project back to the user, use the Studio project URL:
...
Declare variables on the `<html>` root with `data-composition-variables` — a JSON array of declarations (`{id, type, label, default}`). Override with `--variables '{"title":"Q4 Report"}'` at render time.

## Asset Preprocessing

- `npx hyperframes tts` — generate TTS narration audio
- `npx hyperframes transcribe` — produce word-level timestamps
- `npx hyperframes remove-background` — transparent video output

See the `hyperframes-media` skill for voice selection, model rules, and output formats.
